...
void main() {
  ...
  testWidgets(
      'SEARCH: compact header eliminates BackdropFilter to prevent IndexedStack compositing failure',
      (tester) async {
    final container = await pumpSearch(tester);
    addTearDown(container.dispose);

    // Verify BackdropFilter is NOT present in the compact header (prevents IndexedStack compositing failure)
    final backdropFilterFinder = find.byType(BackdropFilter);
    expect(backdropFilterFinder, findsNothing);

    // Verify header AnimatedSlide exists
    final slideFinder = find.byType(AnimatedSlide);
    expect(slideFinder, findsWidgets);

    // Verify header AnimatedOpacity (250ms) exists
    final headerAnimatedOpacityFinder = find.byWidgetPredicate(
